...
def feet_parallel_x(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """
    Penalize front/back distance between left and right wheels.
    
    Returns:
        Tensor of shape (env_num,) representing penalty.
        Higher value → wheels more separated in x-axis.
    """
    asset: RigidObject | Articulation = env.scene[asset_cfg.name]
    
    # 좌/우 발 위치 가져오기
    feet_pos_w = asset.data.body_link_pos_w[:, asset_cfg.body_ids]  # shape: (env_num, 2, 3)
    ...
    feet_pos_b = math_utils.quat_apply_inverse(base_quat, feet_pos_w - base_pos)
    
    # 앞/뒤 거리 계산
    x_distance = torch.abs(feet_pos_b[:, 0, 0] - feet_pos_b[:, 1, 0])
    
    return x_distance  # penalty → RewTerm weight < 0

def joint_pos_l2_zero(env, asset_name, joint_names):
    asset = env.scene[asset_name]
    
    # joint name → index 변환
    joint_indices = [asset.joint_names.index(name) for name in joint_names]
    
    # joint positions 가져오기
...
def track_base_height_command(
    ...
    """
    Track the commanded base height (from command generator).
    """
    asset = env.scene[asset_cfg.name]

    # 명령 불러오기
    cmd = env.command_manager.get_command(command_name)  # shape: [num_envs, 1]
    target_height = cmd.squeeze(-1)
