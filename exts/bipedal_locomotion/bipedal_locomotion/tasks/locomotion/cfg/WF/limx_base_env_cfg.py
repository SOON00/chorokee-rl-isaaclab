...
from bipedal_locomotion.tasks.locomotion.mdp.commands import UniformHeightCommandCfg
@configclass
class CommandsCfg:
...
class ObservarionsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        base_height = ObsTerm(func=mdp.robot_base_height)
...
class RewardsCfg:
    ...
    # rew_nominal_foot_position = RewTerm( # 허리 위치 유지 보상
    ...
    undesired_contacts = RewTerm(
        ...
        weight=-1.25, #-0.25
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=["abad_.*", "hip_.*", "knee_.*", "base_Link"]), "threshold": 0.0}, #10
    )
    ...
    # pen_feet_distance = RewTerm( # Hip yaw 구조에서 불필요
    ...
    # pen_base_height = RewTerm(func=mdp.base_com_height, params={"target_height": 0.50}, weight=-30.0) #로봇 몸체 높이 유지 페널티 -> 학습에 방해될 수 있음
    ...
    # pen_abad_pos = RewTerm(
    #     func=mdp.joint_pos_l2_zero,
    #     weight=-5.0,
    #     params={
    #         "asset_name": "robot",
    ...
    #     },
    # )
    # pen_abad_vel = RewTerm(
    #     func=mdp.joint_vel_l2,
    #     weight=-0.5,  # 속도 제어 (너무 빠른 움직임 방지)
    #     params={"asset_cfg": SceneEntityCfg("robot", joint_names=["abad_L_Joint", "abad_R_Joint"])},
    # )

    # 앞뒤 벌어짐 방지
    pen_wheel_parallel = RewTerm(
        func=mdp.feet_parallel_x,
        weight=-5.0,  # 패널티 강도, 필요시 조절
        params={"asset_cfg": SceneEntityCfg("robot", body_names=["wheel_L_Link", "wheel_R_Link"])},
    )
