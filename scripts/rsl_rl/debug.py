...
class CommandDebugWrapper:
    ...
    def step(self, action):
        ...
        if self._step_count % 10 == 0:
            if hasattr(self.env, "commands") and hasattr(self.env.commands, "base_height"):
                print("Base height command sample:", self.env.commands.base_height.command[:5])
...
def main():
    ...
    env_inner = CommandDebugWrapper(env)  # 먼저 실제 env 감싸기
    env = RslRlVecEnvWrapper(env_inner)   # 그 다음 VecEnv wrapper 적용
